...
C4Component
    ...
    Container_Boundary(server_pkg, "internal/server") {
        ...
        Component(memory_handlers, "Memory Handlers", "handlers_memories.go", "Memória Cronológica: POST /api/memories (idempotency_key), GET/PATCH /api/memories/{MEM-id}, GET /api/memories (lista da árvore, só sessão). Middleware tokenOrSession: Bearer PKD_IMPORT_TOKEN em tempo constante ou sessão; Bearer errado → 401.")
...
| handlers_memories | `server/handlers_memories.go` | `POST /api/memories`, `GET`/`PATCH /api/memories/{MEM-id}`, `GET /api/memories`; middleware `tokenOrSession` |
| MemoryStore | `store/memories.go` | `MemoryDate.Validate`, emissão do ID (`MEM-…-SUFIXO6`, ADR-007), `NormalizeMemoryID`, `CreateMemory` (idempotência + retry de colisão), `GetByMemoryID`, `UpdateMemoryDate` (ID não muda), `ListMemories` (ordem da árvore da MC), guards de hierarquia (`ErrMemoryHierarchy`) |
