...
C4Container
    ...
    System_Boundary(pkd_system, "PKD Docker Container") {
        ...
        Container(svelte_spa, "Svelte 5 SPA", "Svelte 5 + Vite + TipTap v2 + D3.js", "Single-page application compilada em build-time pelo Vite e incorporada ao binário Go via //go:embed. Provê toda a UI: editor, grafo, sidebar, busca, admin, PWA.")
    }
...
|---|---|---|
| **Go HTTP Server** | Auth, sessões, CSRF, CSP, HSTS, rate limiting, todos os 20+ endpoints REST, sanitização HTML (bluemonday), extração Open Graph (x/net/html), serve SPA embutida | Go 1.25, chi v5, modernc.org/sqlite, bluemonday |
| **Svelte 5 SPA** | Editor TipTap com `[[link]]` autocomplete, Graph View D3.js, sidebar com árvore e filtro de tags, busca, calendário, admin, share dialog, tema claro/escuro, PWA install | Svelte 5, Vite 6, TipTap v2, D3.js (modular) |
