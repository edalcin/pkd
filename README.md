...
| `PKD_EMBED_MODEL` | não | `models/gemini-embedding-001` | Modelo Gemini para geração de embeddings. Valores aceitos: `models/gemini-embedding-2` e `models/gemini-embedding-001` — qualquer outro valor é ignorado no boot, com log, caindo para o default (não impede a subida). Alterar o modelo apaga todos os embeddings em cache (re-embed automático na próxima varredura); enquanto ela não termina, a busca opera só com o léxico. Sobreposto pelo valor salvo no DB via admin |
